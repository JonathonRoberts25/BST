...
public class BinarySearchTree<KeyType extends Comparable<KeyType>, ValueType> implements Map<KeyType, ValueType> {
	...
	private KeyType getMinKey(BSTNode root) {
		...
		if(root.left==null){
			...
			return this.getMinKey(root.left);
		}
	}
	...
	private BSTNode remove(KeyType key, BSTNode root) {
		...
		if(key.equals(tKey)) {
			...
			KeyType seckey =this.getMinKey(root.right);
			root.value =this.get(seckey,root.right);
			root.key =seckey;
			root.right=this.remove(seckey,root.right);
		}else if(this.inOrderKeys(key,tKey)){
			this.tSize++;
	...
	public String toString() {
		ArrayList<KeyType> aKeys=this.getAllKeysInOrder();
